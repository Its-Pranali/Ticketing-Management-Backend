...
class OrganizationController extends Controller
{
    ...
    {
        $data = Organization::get()->where('status', 1);
        if (!empty($data)) {
    ...
    public function organizationList(Request $request)
    {
        ...
        $query = Organization::where('status', 1);

        if (!empty($search)) {
            $query->where(function ($q) use ($search) {
                $q->where('org_name', 'like', "%{$search}%")
                    ->orWhere('org_name_ll', 'like', "%{$search}%");
            });
        }
    ...
    public function save(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'org_name_ll' => 'required|string|max:255',
        ]);
        ...
        $data->org_name_ll = $request->org_name_ll;
        $data->status = 1;
